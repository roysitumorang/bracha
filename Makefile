...
build-debug:
	...
	@CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -race -x -ldflags \
		"-X github.com/roysitumorang/bracha/config.AppName=$(PROJECTNAME) \
		-X github.com/roysitumorang/bracha/config.Commit=$(shell git rev-list -1 HEAD) \
		-X github.com/roysitumorang/bracha/config.Build=$(shell date +%FT%T%:z)"
